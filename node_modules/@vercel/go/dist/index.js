...
async function downloadGo(dir = GO_DIR, version = '1.13.7', platform = process.platform, arch = process.arch) {
    ...
    const isUserGo = await fs_extra_1.pathExists(path_1.join(os_1.homedir(), 'go'));
    ...
    if (isUserGo || process.env.GOPATH !== undefined) {
        const { stdout } = await execa_1.default('go', ['version']);
        if (parseInt(stdout.split('.')[1]) >= 11) {
            return createGo(dir, platform, arch);
        }
        throw new Error(`Your current ${stdout} doesn't support Go Modules. Please update.`);
    }
    else {
...
const semver = __webpack_require__(286);
...
const build_utils_2 = __importDefault(__webpack_require__(256));
const { glob, download, createLambda, getWriteableDirectory, debug, } = build_utils_2.default;
...
const errname = __webpack_require__(149);
